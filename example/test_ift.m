...
fprintf(fid,'Molar Volume in liquid phase [Mol/m-3]*10^5=  ');fprintf(fid, '%6.8f\n', system1.getPhase(1).getMolarVolume());
fprintf(fid,'Molar Volume in vapor phase [Mol/m-3]*10^5=  ');fprintf(fid, '%6.8f\n', system1.getPhase(0).getMolarVolume());
...
fprintf(fid,'Mole Density in liquid phase [Mol/m-3]=  ');fprintf(fid, '%6.3f\t %6.3f\n',den_liq_liqht,den_liq_ref);
...
gas_constant=thermo.ThermodynamicConstantsInterface.R; %Gas constant
NA=thermo.ThermodynamicConstantsInterface.avagadroNumber; %avogadro constant
for i=1:2
...
pre_inter_old=pressure_equi;
    ...
    system2 =thermo.system.SystemSrkEos(310.95,108.000);
  ...
  system_newton= thermo.system.SystemSrkEos(310.95,108.000);
...
while (abs(err_den_2)>1e-6)
   ...
   fprintf(fid, '%3i\t', j);fprintf(fid,'Density 1 in interface =  ');fprintf(fid, '%6.3f\t', den_interface_1(j));
...
system_NA_equi = thermo.system.SystemSrkEos(310.95,108.000);
...
for j=1:500
    system_NA(j) = thermo.system.SystemSrkEos(310.95,108.00);
